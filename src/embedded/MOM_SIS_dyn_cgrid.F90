...
subroutine SIS_C_dynamics(ci, mis, mice, ui, vi, uo, vo, fxat, fyat, &
                          sea_lev, fxoc, fyoc, dt_slow, G, US, CS, corrector_step)
...
!$OMP end parallel
  ...
  if (CS%debug_EVP .and. CS%debug) then
    call hchksum(CS%str_d, "str_d pre-EVP SIS_C_dynamics", G%HI, haloshift=1, scale=US%RZ_to_kg_m2*US%L_T_to_m_s**2)
    call hchksum(CS%str_t, "str_t pre-EVP SIS_C_dynamics", G%HI, haloshift=1, scale=US%RZ_to_kg_m2*US%L_T_to_m_s**2)
    call Bchksum(CS%str_s, "str_s pre-EVP SIS_C_dynamics", G%HI, &
                 haloshift=0, symmetric=.true., scale=US%RZ_to_kg_m2*US%L_T_to_m_s**2)
  endif
  if (CS%debug_EVP .and. (CS%debug .or. CS%debug_redundant)) then
    call uvchksum("f[xy]ic  pre-EVP SIS_C_dynamics", fxic, fyic, G, scale=US%RZ_T_to_kg_m2s*US%L_T_to_m_s)
    call uvchksum("f[xy]oc  pre-EVP SIS_C_dynamics", fxoc, fyoc, G, scale=US%RZ_T_to_kg_m2s*US%L_T_to_m_s)
    call uvchksum("f[xy]lf  pre-EVP SIS_C_dynamics", fxlf, fylf, G, scale=US%RZ_T_to_kg_m2s*US%L_T_to_m_s)
    call uvchksum("Cor_[uv] pre-EVP SIS_C_dynamics", Cor_u, Cor_v, G, scale=US%L_T_to_m_s*US%s_to_T)
    call uvchksum("[uv]i    pre-EVP SIS_C_dynamics", ui, vi, G, scale=US%L_T_to_m_s)
    call uvchksum("[uv]_IC    pre-EVP SIS_C_dynamics", u_IC, v_IC, G, scale=US%L_T_to_m_s)
  endif
...
!$OMP                                  m_vio_explicit,b_vel0,vio_pred,vio_C)
    ...
    if (CS%debug_EVP .and. (CS%debug .or. CS%debug_redundant)) then
      call uvchksum("[uv]i in SIS_C_dynamics", ui, vi, G, scale=US%L_T_to_m_s)
      call uvchksum("[uv]o in SIS_C_dynamics", uo, vo, G, scale=US%L_T_to_m_s)
      call uvchksum("mi[uv] in SIS_C_dynamics", mi_u, mi_v, G, scale=US%R_to_kg_m3*US%Z_to_m)
      ...
      !call uvchksum("[uv]io in SIS_C_dynamics", uio_C, vio_C, G, scale=US%L_T_to_m_s)
    endif
...
!$OMP end parallel
  ...
  if (corr_step) then !.and. query_SIS_averaging_enabled(CS%diag)) then
...
!  (ocean & ice), whereas fxat and fyat here are only averaged over the ice.
    ...
    call uvchksum("[uv]i by diags", ui, vi, G, scale=US%L_T_to_m_s)
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~!
!subroutine SIS_C_dyn_diags(ci, mis, mice, ui, vi, uo, vo, fxat, fyat, &
